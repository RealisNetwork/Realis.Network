	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			...
			Exposure {
				...
				// Allow error rate 1
				total_power: Staking::power_of(&11) + ring_power(125) + 1,
				others: vec![IndividualExposure {
					who: 101,
					ring_balance: 125,
					kton_balance: 0,
					// Allow error rate 1
					power: ring_power(125) + 1,
				}]
			}
		);
		...
		assert_eq!(
			...
			Exposure {
				...
				// Allow error rate 1
				total_power: Staking::power_of(&21) + ring_power(375) - 1,
				others: vec![IndividualExposure {
					who: 101,
					// Allow error rate 1
					ring_balance: 375 - 1,
		...
		.execute_with(|| {
			...
			assert_eq!(
				...
				Exposure {
					...
					total_power: ring_power(1000) + ring_power(800),
					others: vec![
						IndividualExposure {
							who: 3,
							ring_balance: 400,
							kton_balance: 0,
							power: ring_power(400),
						},
						IndividualExposure {
							who: 1,
							ring_balance: 400,
			...
			assert_eq!(
				...
				Exposure {
					...
					own_kton_balance: 0,
					own_power: ring_power(1000),
					total_power: ring_power(1000) + ring_power(1200),
					others: vec![
						IndividualExposure {
							who: 3,
...
fn cannot_reserve_staked_balance() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		// Confirm account 11 (via controller 10) is totally staked
		assert_eq!(
			Staking::eras_stakers(active_era(), &11).own_ring_balance,
			1000,
		);
		// Confirm account 11 cannot transfer as a result
...
fn bond_with_no_staked_value() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Ring::locks(&1)[0].locked_amount(Some(System::block_number())),
				5
			);
					...
					(
						...
						Support {
							...
							voters: vec![
								...
								(3, ring_power(400) as _),
								(1, ring_power(400) as _)
							]
						}
					),
					(
						31,
						Support {
							total: ring_power(2200) as _,
							voters: vec![
								(31, ring_power(1000) as _),
								(3, ring_power(600) as _),
								(1, ring_power(600) as _)
							]
						}
					)
				],
...
fn unbonded_balance_is_not_slashable() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(Staking::unbond(
			Origin::signed(10),
			StakingBalance::RingBalance(800)
		));
...
fn cannot_rebond_to_lower_than_ed() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Ring::free_balance(&21), 512_000);
