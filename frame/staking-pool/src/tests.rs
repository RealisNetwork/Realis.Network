...
fn nominating_and_rewards_should_work() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            mock::make_all_reward_payment(0);
            assert_eq!(
                Balances::total_balance(&40),
                initial_balance_40 + total_payout_0 / 2
            );
            assert_eq!(
                Balances::total_balance(&20),
                initial_balance_20 + total_payout_0 / 2
            );
...
fn reap_stash_works() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_noop!(
                Staking::reap_stash(Origin::signed(20), 10, 0),
                Error::<Test>::NotStash
            );
...
fn wrong_vote_is_moot() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert!(Staking::eras_stakers(active_era(), 11)
                .others
                .iter()
                .any(|i| i.who == 61));
            assert!(Staking::eras_stakers(active_era(), 21)
                .others
                .iter()
                .any(|i| i.who == 61));
        });
...
fn garbage_collection_after_slashing() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(
                <Staking as crate::Store>::SpanSlash::get(&(11, 0)).amount(),
                &200
            );
...
mod sorted_list_provider {
    ...
    fn re_nominate_does_not_change_counters_or_list() {
        ExtBuilder::default().nominate(true).build_and_execute(|| {
            ...
            assert_eq!(
                Nominators::<Test>::iter().count() as u32,
                pre_insert_nominator_count
            );
