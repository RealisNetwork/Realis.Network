...
use super::mock::{
    ...
    Erc721, Erc721Id, Event, HashId, NativeTokenId, Origin, ProposalLifetime, RealisBridge, Test,
...
use pallet_nft::Erc721Token;

const TEST_THRESHOLD: u32 = 2;

fn make_remark_proposal(hash: H256) -> Call {
    let resource_id = HashId::get();
    Call::RealisBridge(crate::Call::remark(hash, resource_id))
}

fn make_transfer_proposal(to: u64, amount: u64) -> Call {
    let resource_id = HashId::get();
    Call::RealisBridge(crate::Call::transfer(to, amount, resource_id))
}
...
fn transfer_hash() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(RealisBridge::transfer_hash(
            Origin::signed(1),
            hash,
            dest_chain,
        ));
...
fn transfer_native() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Bridge::whitelist_chain(Origin::root(), dest_chain));
        assert_ok!(RealisBridge::transfer_native(
...
fn transfer_erc721() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(RealisBridge::transfer_erc721(
            Origin::signed(RELAYER_A),
            recipient.clone(),
            token_id,
            dest_chain,
...
fn execute_remark() {
    new_test_ext().execute_with(|| {
        let hash: H256 = "ABC".using_encoded(blake2_256).into();
        let proposal = make_remark_proposal(hash);
        let prop_id = 1;
        let src_id = 1;
        let r_id = bridge::derive_resource_id(src_id, b"hash");
        let resource = b"RealisBridge.remark".to_vec();
...
fn execute_remark_bad_origin() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            RealisBridge::remark(Origin::signed(RELAYER_A), hash, resource_id),
            DispatchError::BadOrigin
        );
        // Don't allow root calls
        assert_noop!(
            RealisBridge::remark(Origin::root(), hash, resource_id),
            DispatchError::BadOrigin
        );
    })
}
...
fn transfer() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(RealisBridge::transfer(
            Origin::signed(Bridge::account_id()),
            RELAYER_A,
            10,
            resource_id,
...
fn mint_erc721() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Erc721::tokens(token_id), None);
        // Mint
        assert_ok!(RealisBridge::mint_erc721(
...
fn create_sucessful_transfer_proposal() {
    new_test_ext().execute_with(|| {
        ...
        let resource = b"RealisBridge.transfer".to_vec();
