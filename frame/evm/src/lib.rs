...
use frame_support::traits::{
    Currency, ExistenceRequirement, FindAuthor, Imbalance, OnUnbalanced,
    SameOrOther::{None, Other, Same},
    WithdrawReasons,
};
...
pub mod pallet {
    ...
    where
        OuterOrigin:
            Into<Result<RawOrigin<AccountId32>, OuterOrigin>> + From<RawOrigin<AccountId32>>,
    {
        ...
        fn try_address_origin(
            ...
            origin.into().and_then(|o| match o {
                RawOrigin::Signed(who)
                    if AsRef::<[u8; 32]>::as_ref(&who)[0..20] == address[0..20] =>
                {
                    Ok(who)
                }
                r => Err(OuterOrigin::from(r)),
    ...
    pub(crate) type AccountStorages<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, H160, Blake2_128Concat, H256, H256>;
    ...
    #[pallet::metadata(T::AccountId = "AccountId", Log = "Log", H160 = "H160", U256 = "U256")]
    ...
    impl<T: Config> Pallet<T> {
        ...
        #[pallet::weight(T::GasWeightMapping::gas_to_weight(0))]
        pub fn withdraw(
            origin: OriginFor<T>,
            address: H160,
            value: BalanceOf<T>,
        ...
        pub fn call(
            ...
            match info.exit_reason {
                ExitReason::Succeed(_) => {
                    Pallet::deposit_event(Event::<T>::Executed(target));
                }
                _ => {
                    Pallet::deposit_event(Event::<T>::ExecutedFailed(target));
                }
            };
        ...
        pub fn create(
            ...
            match info {
                CreateInfo {
                    ...
                    Pallet::deposit_event(Event::<T>::Created(create_address));
                }
                CreateInfo {
                    exit_reason: _,
                    value: create_address,
                    ...
                    Pallet::deposit_event(Event::<T>::CreatedFailed(create_address));
                }
            }
        ...
        fn correct_and_deposit_fee(
            ...
            if let Some(paid) = already_withdrawn {
                ...
                let adjusted_paid = match paid.offset(refund_imbalance) {
                    ...
                    None | Other(_) => Err(Error::<T>::BalanceLow),
                }
                .map_err(|_| Error::<T>::BalanceLow)?;
