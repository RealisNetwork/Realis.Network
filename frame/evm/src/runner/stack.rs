...
use crate::pallet::{AddressMapping, BlockHashMapping, FeeCalculator, OnChargeEVMTransaction};
...
impl<T: EVM::Config> Runner<T> {
    ...
    pub fn execute<'config, F, R>(
        ...
        f: F,
    ) -> Result<ExecutionInfo<R>, EVM::Error<T>>
    where
        F: FnOnce(
        ...
        ) -> (ExitReason, R),
    {
        ...
        let total_payment = value
            ...
            .ok_or(EVM::Error::<T>::PaymentOverflow)?;
        let source_account = EVM::Pallet::<T>::account_basic(&source);
        ensure!(
            source_account.balance >= total_payment,
            EVM::Error::<T>::BalanceLow
        );

        if let Some(nonce) = nonce {
            ensure!(source_account.nonce == nonce, EVM::Error::<T>::InvalidNonce);
        }
...
impl<'vicinity, 'config, T: EVM::Config> SubstrateStackState<'vicinity, 'config, T> {
...
impl<'vicinity, 'config, T: EVM::Config> BackendT for SubstrateStackState<'vicinity, 'config, T> {
...
impl<'vicinity, 'config, T: EVM::Config> StackStateT<'config>
    for SubstrateStackState<'vicinity, 'config, T>
{
    ...
    fn set_code(&mut self, address: H160, code: Vec<u8>) {
        ...
        EVM::Pallet::<T>::create_account(address, code);
    }
