...
impl<T: EVM::Config> Runner<T> {
	...
	pub fn execute<'config, F, R>(
		...
		f: F,
	) -> Result<ExecutionInfo<R>, EVM::Error<T>> where
		F: FnOnce(&mut StackExecutor<'config, SubstrateStackState<'_, 'config, T>>) -> (ExitReason, R),
	{
		// Gas price check is skipped when performing a gas estimation.
		let gas_price = match gas_price {
			Some(gas_price) => {
				ensure!(gas_price >= T::FeeCalculator::min_gas_price(), EVM::Error::<T>::GasPriceTooLow);
				gas_price
			},
...
impl<'vicinity, 'config, T: EVM::Config> StackStateT<'config> for SubstrateStackState<'vicinity, 'config, T> {
